...
Status MiniOidc::Start() {
  ...
  Sockaddr addr;
  RETURN_NOT_OK(jwks_server_->GetAdvertisedAddresses(&bound_addrs));
  ...
  const string jwks_url = Substitute("$0://localhost:$1/jwks", protocol, bound_addrs[0].port());
  ...
  bound_addrs.clear();
  RETURN_NOT_OK(oidc_server_->GetAdvertisedAddresses(&bound_addrs));
  RETURN_NOT_OK(addr.ParseString(bound_addrs[0].host(), bound_addrs[0].port()));
