...
const std::string kId1 = "public:c424b67b-fe28-45d7-b015-f79da50b5b21";
const std::string kId2 = "public:9b9d0b47-b9ed-4ba6-9180-52fc5b161a3a";
...
TEST(JwtUtilTest, LoadJwksFile) {
  // Load JWKS from file.
  TempTestDataFile jwks_file(Substitute(kJwksRsaFileFormat, kId1, "RS256",
      kRsaPubKeyJwkN, kRsaPubKeyJwkE, kId2, "RS256", kRsaInvalidPubKeyJwkN,
      kRsaPubKeyJwkE));
  ...
  const JWTPublicKey* key1 = jwks->LookupRSAPublicKey(kId1);
...
TEST(JwtUtilTest, LoadInvalidJwksFiles) {
  ...
  jwks_file.reset(new TempTestDataFile(
      Substitute(kJwksRsaFileFormat, kId1, "RS256", "", "", kId2, "RS256", "", "")));
  status = jwt_helper.Init(jwks_file->Filename(), true);
  ASSERT_FALSE(status.ok());
  ASSERT_STR_CONTAINS(status.ToString(), "parsing key #0")
      << " Actual error: " << status.ToString();
...
TEST(JwtUtilTest, VerifyJwtHS256) {
  ...
  TempTestDataFile jwks_file(
      Substitute(kJwksHsFileFormat, kId1, "HS256", shared_secret));
  JWTHelper jwt_helper;
  Status status = jwt_helper.Init(jwks_file.Filename(), true);
  EXPECT_OK(status);
  JWKSSnapshotPtr jwks = jwt_helper.GetJWKS();
  ...
  const JWTPublicKey* key1 = jwks->LookupHSKey(kId1);
  ASSERT_TRUE(key1 != nullptr);
...
TEST(JwtUtilTest, VerifyJwtHS384) {
  ...
  TempTestDataFile jwks_file(
      Substitute(kJwksHsFileFormat, kId1, "HS384", shared_secret));
  JWTHelper jwt_helper;
  Status status = jwt_helper.Init(jwks_file.Filename(), true);
  EXPECT_OK(status);
  JWKSSnapshotPtr jwks = jwt_helper.GetJWKS();
...
TEST(JwtUtilTest, VerifyJwtHS512) {
  ...
  TempTestDataFile jwks_file(
      Substitute(kJwksHsFileFormat, kId1, "HS512", shared_secret));
  JWTHelper jwt_helper;
  Status status = jwt_helper.Init(jwks_file.Filename(), true);
  EXPECT_OK(status);
  JWKSSnapshotPtr jwks = jwt_helper.GetJWKS();
...
TEST(JwtUtilTest, VerifyJwtRS384) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksRsaFileFormat, kId1, "RS384",
      kRsaPubKeyJwkN, kRsaPubKeyJwkE, kId2, "RS384", kRsaInvalidPubKeyJwkN,
      kRsaPubKeyJwkE));
  JWTHelper jwt_helper;
  Status status = jwt_helper.Init(jwks_file.Filename(), true);
  EXPECT_OK(status);
...
TEST(JwtUtilTest, VerifyJwtRS512) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksRsaFileFormat, kId1, "RS512",
      kRsa512PubKeyJwkN, kRsa512PubKeyJwkE, kId2, "RS512",
...
TEST(JwtUtilTest, VerifyJwtPS256) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksRsaFileFormat, kId1, "PS256",
      kRsa1024PubKeyJwkN, kRsa1024PubKeyJwkE, kId2, "PS256",
...
TEST(JwtUtilTest, VerifyJwtPS384) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksRsaFileFormat, kId1, "PS384",
      kRsa2048PubKeyJwkN, kRsa2048PubKeyJwkE, kId2, "PS384",
      kRsaInvalidPubKeyJwkN, kRsaPubKeyJwkE));
  JWTHelper jwt_helper;
  Status status = jwt_helper.Init(jwks_file.Filename(), true);
  EXPECT_OK(status);
...
TEST(JwtUtilTest, VerifyJwtPS512) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksRsaFileFormat, kId1, "PS512",
      kRsa4096PubKeyJwkN, kRsa4096PubKeyJwkE, kId2, "PS512",
      kRsaInvalidPubKeyJwkN, kRsaPubKeyJwkE));
  JWTHelper jwt_helper;
  Status status = jwt_helper.Init(jwks_file.Filename(), true);
  EXPECT_OK(status);
...
TEST(JwtUtilTest, VerifyJwtES256) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksEcFileFormat, kId1, "P-256",
  ...
  const JWTPublicKey* key1 = jwks->LookupECPublicKey(kId1);
  ASSERT_TRUE(key1 != nullptr);
...
TEST(JwtUtilTest, VerifyJwtES384) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksEcFileFormat, kId1, "P-384",
...
TEST(JwtUtilTest, VerifyJwtES512) {
  ...
  TempTestDataFile jwks_file(Substitute(kJwksEcFileFormat, kId1, "P-521",
