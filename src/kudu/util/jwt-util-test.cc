...
TEST(JwtUtilTest, VerifyJWKSDiscoveryEnpointMultipleClients) {
  JWKSDiscoveryEndpointMockServer discovery_endpoint;
  ASSERT_OK(discovery_endpoint.Start());
  PerAccountKeyBasedJwtVerifier jwt_verifier(discovery_endpoint.url());
  {
    ...
    auto verify = [&] () {
      Status s = jwt_verifier.VerifyToken(valid_user_token, &subject);
      ASSERT_EQ(kValidAccount, subject);
      return;
    };

    RETURN_NOT_OK(Thread::Create("JWT", "JWT-client",
        verify, &jwks_update_thread_));


    std::thread first_client(verify);
    std::thread second_client(verify);
//    std::thread third_client(verify);
