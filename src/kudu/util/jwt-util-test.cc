...
const string kMissingAccount = "no-one";
} // anonymous namespace
...
TEST(JwtUtilTest, VerifyOIDCDiscoveryEndpoint) {
  ...
  unordered_map<string, string> invalid_jwts {
    { oidc.CreateJwt(kInvalidAccount, kSubject, true), "invalid issuer with invalid subject" },
  ...
  for (const auto& [jwt, msg] : invalid_jwts) {
    string subject;
    Status s = jwt_verifier.VerifyToken(jwt, &subject);
