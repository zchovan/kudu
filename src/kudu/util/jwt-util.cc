...
DEFINE_validator(jwks_update_frequency_s,
                 [](const char* name, const int32_t val) {
                     if (val <= 0) {
...
DEFINE_validator(jwks_pulling_timeout_s,
                 [](const char* name, const int32_t val) {
                     if (val <= 0) {
                       LOG(ERROR) << Substitute("Invalid value for $0 flag: $1", name, val);
                       return false;
...
bool RSAJWTPublicKeyBuilder::ConvertJwkToPem(
  ...
  BIGNUM* modul = BN_bin2bn(reinterpret_cast<const unsigned char*>(str_n.c_str()), static_cast<int>(str_n.size()), nullptr);
  BIGNUM* expon = BN_bin2bn(reinterpret_cast<const unsigned char*>(str_e.c_str()), static_cast<int>(str_e.size()), nullptr);
...
bool ECJWTPublicKeyBuilder::ConvertJwkToPem(int eccgrp, const std::string& base64_x,
  ...
  BIGNUM* x = BN_bin2bn(reinterpret_cast<const unsigned char*>(ascii_x.c_str()), static_cast<int>(ascii_x.size()), nullptr);
  BIGNUM* y = BN_bin2bn(reinterpret_cast<const unsigned char*>(ascii_y.c_str()), static_cast<int>(ascii_y.size()), nullptr);
...
void JWKSMgr::UpdateJWKSThread() {
  ...
  int64_t timeout_millis = static_cast<int64_t>(FLAGS_jwks_update_frequency_s) * 1000;
  while (true) {
    bool timed_out = shut_down_promise_.WaitFor(MonoDelta::FromMilliseconds(timeout_millis));
...
Status JWTHelper::Decode(const string& token, UniqueJWTDecodedToken& decoded_token_out) {
  ...
  try {
    ...
    for (auto& claim : decoded_token_out.get()->decoded_jwt_.get_header_claims()) {
