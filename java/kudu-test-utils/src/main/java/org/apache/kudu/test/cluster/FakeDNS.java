  ...
  public synchronized void install() {
    ...
    try {
      try {
        ...
        Class<?> nameServiceInterface = null;
        for (Class<?> innerClass : InetAddress.class.getDeclaredClasses()) {
          if ("NameService".equals(innerClass.getSimpleName())) {
            nameServiceInterface = innerClass;
            break;
          }
        }
        if (nameServiceInterface == null) {
          throw new ClassNotFoundException("java.net.InetAddress$NameService");
        }
      ...
      } catch (ReflectiveOperationException | RuntimeException modernError) {
        // Override InetAddress resolver in Java 25+.
        try {
          Class<?> resolverInterface = Class.forName("java.net.spi.InetAddressResolver");
  ...
  private class NameServiceListener implements InvocationHandler {
    ...
    private InetAddress[] lookupAllHostAddr(String host) throws UnknownHostException {
      ...
      try {
        ...
        try {
          Class<?> lookupPolicyClass = Class.forName("java.net.spi.InetAddressResolver$LookupPolicy");
          Method method = fallbackNameService.getClass()
